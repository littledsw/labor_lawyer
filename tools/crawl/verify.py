...
def main() -> int:
    ...
    problems: list[str] = []
    # 覆盖全部地区（national + municipalities/*），不只北京；新增地区自动纳入校验
    regions_root = REPO / "regions"
    roots = [regions_root] if regions_root.exists() else []
    disk = {str(p.relative_to(REPO)) for root in roots
            for p in root.rglob("*")
